...
import { itemDisplayText } from "./itemDisplay";
...
export const ItemListItem = memo(function ItemListItem({ item, onClick, onDoubleClick, onLocate, onRename, onDelete, onStack, dragging }: {
  ...
  const contextActive = overflowOpen || Boolean(sendAnchor);
  ...
  return <ListItem disablePadding secondaryAction={inView ? <><HierarchyActionRow actions={actions} context={{ label: itemDisplayText(item, role), icon: <ItemIcon item={item} /> }} onOverflowOpenChange={setOverflowOpen} />{hasUpdatePermission && <SendMenuButton hideButton externalAnchor={sendAnchor} itemIds={ids} onStack={(operation) => onStack?.(ids, operation)} onOpenChange={(open) => { if (!open) setSendAnchor(null); }} />}</> : undefined} sx={{ ".MuiListItemButton-root": { pr: inView ? `${actionColumns * slotSize + 22}px` : undefined } }}>
    <ListItemButton ref={ref} selected={selected} dense onClick={onClick} onDoubleClick={onDoubleClick} sx={{ minHeight: slotSize, margin: "4px 8px", borderRadius: "12px", backgroundColor: dragging ? `${theme.palette.primary.main} !important` : contextActive ? "action.hover" : "background.default", boxShadow: dragging ? theme.shadows[5] : contextActive ? `inset 0 0 0 2px ${theme.palette.info.main}` : undefined, color: dragging ? `${theme.palette.primary.contrastText} !important` : selected ? "primary.main" : undefined, borderLeft: "3px solid", borderLeftColor: selected ? "primary.main" : "transparent", cursor: dragging ? "grabbing" : undefined }}>
